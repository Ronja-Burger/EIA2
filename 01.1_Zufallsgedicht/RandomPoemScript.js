...
(function (RandomPoem) {
    ...
    for (let index = 5; index >= 1; index--) {
        let solution = getVerse(subjekte, praedikate, objekte);
        //console.log(index);
        console.log(solution);
    }
    function getVerse(_subjekte, _prädikate, _objekte) {
        //creation of numbers: SUBJECT
        let zahlSub = Math.floor(Math.random() * _subjekte.length);
        //console.log(subZahl);
        // Zufallswert aus den Subjekten
        let removedSubject = subjekte.splice(zahlSub, 1);
        //console.log(removedSubject + " ");
        //creation of numbers: PRÄDIKAT
        let zahlPra = Math.floor(Math.random() * _prädikate.length);
        //console.log(präZahl);
        // Zufallswert aus den Prädikaten
        let removedPraedikat = praedikate.splice(zahlPra, 1);
        //console.log(removedPrädikat + " ");
        //creation of numbers: OBJEKT
        let zahlObj = Math.floor(Math.random() * _objekte.length);
        //console.log(objZahl);
        // Zufallswert aus den Objekten
        let removedObject = objekte.splice(zahlObj, 1);
        //console.log(removedObject + " ");
        let einVers = removedSubject[0] + " " + removedPraedikat[0] + " " + removedObject[0];
        //console.log(einVers);
        return einVers;
    }
